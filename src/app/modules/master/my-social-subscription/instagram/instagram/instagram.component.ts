...
export class InstagramComponent implements OnInit {
  ...
  instagramSubForm: FormGroup = new FormGroup({});
  ...
  constructor(private modalService: BsModalService,
              ...
              private SocialInstagramService: SocialInstagramService,
  ...
  ngOnInit(): void {
    ...
    this._activatedRoute.queryParams.subscribe(params => {
      if (params['social_name']) {
        ...
        this.social_name = this.socialNames.instagram;
      }
  ...
  async getSocialFeed(filter: any) {
    ...
    this.SocialInstagramService.getSocialSubData(filter, accessToken).subscribe(socialData => {

      let { instagramData, pageSize, totalTweets } = socialData.body;

      this.pageSize = pageSize;
      let totalTweetsCount = !_.isEmpty(totalTweets) ? totalTweets[0] : { count: 0 };
      this.totalTweets = totalTweetsCount.count;
      this.instagramData = !_.isEmpty(instagramData) ? instagramData : [];
  ...
  async getMySubscriptionList() {
    ...
    this.SocialInstagramService.getMySubscriptionList(accessToken, filter).subscribe(
      mySubscriptionData => {
   ...
   * @param socialName
   */
  socialName = SocialNames.instagram;
  ...
  fetchDataForScreenName(social_unique_identification: any, social_name: any) {
    this.EmptyInstagramUrl = true;
    let basicUrl = `https://www.instagram.com/${social_unique_identification}/embed`;
    this.instagramUrl = this.sanitizer.bypassSecurityTrustResourceUrl(basicUrl);
  ...
  async confirmToDeleteGroup() {
    if (this.subscriptionGroup) {
      let accessToken = await this.awsCognitoAuthenticatedUserService.getAuthenticatedUserToken();
      this.SocialInstagramService.deleteSocialGroup(this.subscriptionGroup, accessToken).subscribe(deletedData => {
  ...
  async confirmToDeleteSub() {
    if (this.subscription) {
      ...
      let accessToken = await this.awsCognitoAuthenticatedUserService.getAuthenticatedUserToken();
      this.SocialInstagramService.deleteMySocialSubData(deleteObject, accessToken).subscribe(deletedData => {
  ...
  /**
   * Initializing instagram subs group form
   */
  initTwitterSubForm() {
    this.instagramSubForm = this.formBuilder.group({
  ...
  openSocialConnect(template: TemplateRef<any>, socialAccountName: any) {
    ...
    this.instagramSubForm.patchValue({ "social_name": socialAccountName });
  ...
  /**
   * Submit instagram form data
   */
  isSubFormSubmitted = false;
  errorMessage = null;
  async submitSubForm() {
    if (this.instagramSubForm.valid && !this.isSubFormSubmitted) {
      this.isSubFormSubmitted = true;
      let subData = this.instagramSubForm.value;
      let { group_name, ...datas } = subData;
      datas["group_name"] = group_name.group_name;
      datas["group_name_id"] = group_name._id;
      let accessToken = await this.awsCognitoAuthenticatedUserService.getAuthenticatedUserToken();
      this.SocialInstagramService.addMySocialSubData(datas, accessToken).subscribe(
        (sub: any) => {
  ...
  async submitAddUserSubsGroupForm() {
    if (this.userGroupSubForm.valid && !this.isUserSubsGroupFormSubmiteed) {
      ...
      let accessToken = await this.awsCognitoAuthenticatedUserService.getAuthenticatedUserToken();
      this.SocialInstagramService.addSocialGroup(subData, accessToken).subscribe(
        (sub: any) => {
  ...
  async addToFavorties(twitterId: any, screenName: any) {
    let accessToken = await this.awsCognitoAuthenticatedUserService.getAuthenticatedUserToken();
    this.SocialInstagramService.addTofavorites({ twitterId, screenName }, accessToken).subscribe(savedfavorites => {
      this.SocialInstagramService.getMySubscriptionList(accessToken, { socialName: this.socialName }).subscribe(
        mySubscriptionList => {
